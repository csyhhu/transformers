...
class AlbertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.ffn = quantized_Linear(config.hidden_size, config.intermediate_size, bitW=config.bitW)
        self.ffn_output = quantized_Linear(config.intermediate_size, config.hidden_size, bitW=config.bitW)
...
class AlbertLayerGroup(nn.Module):
    def __init__(self, config):
        ...
        self.albert_layers = nn.ModuleList([AlbertLayer(config) for _ in range(config.inner_group_num)]) # inner_group_num = 1
...
class AlbertTransformer(nn.Module):
    def __init__(self, config):
        ...
        self.albert_layer_groups = nn.ModuleList([AlbertLayerGroup(config) for _ in range(config.num_hidden_groups)]) # num_hidden_groups = 1
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None):
        ...
        for i in range(self.config.num_hidden_layers): # num_hidden_layers = 12
            ...
            # albert_layer_groups is performed for 12 times
...
class AlbertModel(AlbertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.pooler = quantized_Linear(config.hidden_size, config.hidden_size, bitW=config.bitW)
