...
from utils.miscellaneous import is_int, progress_bar, accuracy, AverageMeter
...
class Recorder():
    ...
    def __init__(self, SummaryPath, dataset_name='CIFAR10', task_name = None):
        ...
        if self.dataset_type == 'small':
            ...
            self.constraint_record = open('%s/%sconstraint.txt' % (self.SummaryPath, prefix), 'w+')
        else:
            self.loss_record = open('%s/%sloss.txt' % (self.SummaryPath, prefix), 'w+')
    ...
    def print_training_result(self, batch_idx, n_batch, monitor_freq=100, append=None):

        if self.dataset_type == 'small':
            progress_bar(batch_idx, n_batch,
                         "Loss: %.3f (%.3f), Acc: %.3f%% (%.3f%%) | %s"
                         % (self.loss.val, self.loss.avg,
                            100.0 * self.top1.val, 100.0 * self.top1.avg,
                            append))
        else:
            # raise NotImplementedError
