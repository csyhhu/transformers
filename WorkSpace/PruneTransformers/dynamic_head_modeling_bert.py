...
class DynamicHeadSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None):
        ...
        # print(mixed_query_layer.shape)  # torch.Size([10, 128, 768])
        ...
        value_layer = self.transpose_for_scores(mixed_value_layer) # torch.Size([10, 12, 128, 64])
        # print('query_layer: ')
        # print(query_layer.shape) # torch.Size([10, 12, 128, 64])
        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        # print(attention_scores.shape) # [10, 12, 128, 128]
        if attention_mask is not None:
            # Apply the attention mask is (precomputed for all layers in BertModel forward() function)
            attention_scores = attention_scores + attention_mask

        # Normalize the attention scores to probabilities.
        attention_probs = nn.Softmax(dim=-1)(attention_scores) # [10, 12, 128, 128]
